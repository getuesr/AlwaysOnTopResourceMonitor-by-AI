...
try:
    ...
    from PyQt5.QtWidgets import (
        ...
        QScrollArea, QSpinBox, QRadioButton, QButtonGroup, QFontDialog
    )
...
DEFAULT_CONFIG = {
    ...
    "font_bold": False,
    "name_bg_color": None,
    "custom_name_bg_colors": {},
    "global_remark": "",
    "global_remark_enabled": False,
    "no_global_remark_for_remarked": True
}
...
class ScrollingLabel(QLabel):
    ...
    def apply_config_font(self, family, size, bold):
        font = QFont(family if family else self.default_font().family(), size)
        font.setBold(bold)
        self.setFont(font)
...
class NameBgColorEditDialog(QDialog):
    def __init__(self, parent=None, data=None):
        super().__init__(parent)
        self.data = data or {}
        self.setWindowTitle("程序名背景色设置")
        self.setWindowFlags(Qt.Dialog | Qt.WindowCloseButtonHint)
        self.resize(400, 200)
        self.bg_color = self.data.get("bgColor", "#2A2A2A")
        self.corner_radius = self.data.get("cornerRadius", 6)
        self.initUI()
    ...
    def initUI(self):
        ...
        self.radius_slider.setValue(self.corner_radius)
        ...
        layout.addRow("圆角程度:", radius_layout)
        btn_box = QHBoxLayout()
        btn_ok = QPushButton("确定")
        btn_cancel = QPushButton("取消")
        btn_ok.clicked.connect(self.accept)
    ...
    def get_data(self):
        return {
            "proc": self.edit_proc.text().strip(),
            "bgColor": self.bg_color,
            "cornerRadius": self.radius_slider.value()
        }

class NameBgColorManagerDialog(QDialog):
    def __init__(self, colors, parent=None):
        super().__init__(parent)
        self.colors = colors.copy()
        self.setWindowTitle("自定义程序名背景色")
        self.setWindowFlags(Qt.Dialog | Qt.WindowCloseButtonHint)
        self.resize(500, 400)
        self.initUI()
        self.refresh_list()
    ...
    def initUI(self):
        ...
        btn_edit.clicked.connect(self.edit_color)
        btn_delete.clicked.connect(self.delete_color)
        btn_save.clicked.connect(self.save_and_close)
        btn_cancel.clicked.connect(self.reject)
        btn_layout.addWidget(btn_add)
        btn_layout.addWidget(btn_edit)
    ...
    def refresh_list(self):
        self.list_widget.clear()
        for proc, data in self.colors.items():
            self.list_widget.addItem(f"{proc} — 背景:{data['bgColor']} 圆角:{data['cornerRadius']}px")

    def add_color(self):
        dlg = NameBgColorEditDialog(self)
        if dlg.exec_():
            data = dlg.get_data()
            if data["proc"]:
                self.colors[data["proc"]] = {"bgColor": data["bgColor"], "cornerRadius": data["cornerRadius"]}
                self.refresh_list()

    def edit_color(self):
        row = self.list_widget.currentRow()
        if row >= 0:
            proc = list(self.colors.keys())[row]
            data = self.colors[proc]
            dlg = NameBgColorEditDialog(self, {"proc": proc, "bgColor": data["bgColor"], "cornerRadius": data["cornerRadius"]})
            if dlg.exec_():
                new_data = dlg.get_data()
                if new_data["proc"]:
                    del self.colors[proc]
                    self.colors[new_data["proc"]] = {"bgColor": new_data["bgColor"], "cornerRadius": new_data["cornerRadius"]}
                    self.refresh_list()
...
class SettingsWindow(QWidget):
    def __init__(self, cfg, callback, parent=None):
        ...
        self.setFixedSize(560, 980)
        self.remarks = cfg.get("remarks", [])
        self.per_process_offsets = cfg.get("per_process_offsets", {})
        self.custom_name_bg_colors = cfg.get("custom_name_bg_colors", {})
        self.drag_pos = None
        self.initUI()
    ...
    def initUI(self):
        ...
        g_font = QGroupBox("🔤 字体设置")
        g_font.setStyleSheet("color:white; font-weight:bold; border:1px solid #5a5a5a; border-radius:10px; margin-top:12px; padding-top:12px;")
        font_layout = QHBoxLayout(g_font)
        self.font_btn = QPushButton("选择字体")
        self.font_btn.clicked.connect(self.choose_font)
        self.font_label = QLabel(f"当前：{self.cfg.get('font_family', '系统默认')} {self.cfg.get('font_size', 9)}pt {'粗体' if self.cfg.get('font_bold', False) else ''}")
        self.font_label.setStyleSheet("color:#e0e0e0;")
        font_layout.addWidget(self.font_btn)
        font_layout.addWidget(self.font_label)
        clayout.addWidget(g_font)
        ...
        self.cb_use_title = QCheckBox("显示程序名字（窗口标题）")
        self.cb_name = QCheckBox("进程名")
        self.cb_pid = QCheckBox("PID")
        self.cb_cpu = QCheckBox("CPU 占用")
        self.cb_mem = QCheckBox("内存占用")
        self.cb_time = QCheckBox("时间")
        self.cb_upload = QCheckBox("网络上传")
        self.cb_download = QCheckBox("网络下载")
        for cb in [self.cb_use_title, self.cb_name, self.cb_pid, self.cb_cpu, self.cb_mem, self.cb_time, self.cb_upload, self.cb_download]:
            cb.setStyleSheet("color:#f0f0f0;")
            g1lay.addWidget(cb)
        self.cb_use_title.setChecked(self.cfg.get("use_window_title", False))
        ...
        self.cb_upload.setChecked(self.cfg.get("show_upload", False))
        self.cb_download.setChecked(self.cfg.get("show_download", False))
        self.cb_use_title.toggled.connect(self.toggle_title_options)
        ...
        self.title_bg_group = QGroupBox("程序名字背景色")
        self.title_bg_group.setStyleSheet(g1.styleSheet())
        title_bg_layout = QVBoxLayout(self.title_bg_group)
        self.global_name_bg_btn = QPushButton("全局背景色")
        self.global_name_bg_btn.clicked.connect(self.pick_global_name_bg)
        self.global_name_bg_label = QLabel(f"当前：{self.cfg.get('name_bg_color', '未设置')}")
        self.global_name_bg_label.setStyleSheet("color:#e0e0e0;")
        title_bg_layout.addWidget(self.global_name_bg_btn)
        title_bg_layout.addWidget(self.global_name_bg_label)
        btn_manage_name_bg = QPushButton("管理自定义程序背景色...")
        btn_manage_name_bg.clicked.connect(self.open_name_bg_manager)
        title_bg_layout.addWidget(btn_manage_name_bg)
        hint = QLabel("单独设定颜色的程序大于全局设置")
        hint.setStyleSheet("color:#aaa; font-size:11px;")
        title_bg_layout.addWidget(hint)
        clayout.addWidget(self.title_bg_group)
        self.title_bg_group.setVisible(self.cfg.get("use_window_title", False))
        ...
        g_global_remark = QGroupBox("🌐 全局备注")
        g_global_remark.setStyleSheet(g1.styleSheet())
        gr_layout = QVBoxLayout(g_global_remark)
        self.global_remark_enabled_cb = QCheckBox("启用全局备注")
        self.global_remark_enabled_cb.setStyleSheet("color:#f0f0f0;")
        self.global_remark_enabled_cb.setChecked(self.cfg.get("global_remark_enabled", False))
        self.global_remark_enabled_cb.toggled.connect(self.toggle_global_remark)
        gr_layout.addWidget(self.global_remark_enabled_cb)
        gr_input_layout = QHBoxLayout()
        self.global_remark_edit = QLineEdit()
        self.global_remark_edit.setText(self.cfg.get("global_remark", ""))
        self.global_remark_edit.setPlaceholderText("输入全局备注...")
        self.global_remark_edit.setEnabled(self.cfg.get("global_remark_enabled", False))
        self.global_remark_btn = QPushButton("设定")
        self.global_remark_btn.clicked.connect(self.set_global_remark)
        self.global_remark_btn.setEnabled(self.cfg.get("global_remark_enabled", False))
        gr_input_layout.addWidget(self.global_remark_edit)
        gr_input_layout.addWidget(self.global_remark_btn)
        gr_layout.addLayout(gr_input_layout)
        self.no_global_remark_for_remarked_cb = QCheckBox("有备注的程序不使用全局备注")
        self.no_global_remark_for_remarked_cb.setStyleSheet("color:#f0f0f0;")
        self.no_global_remark_for_remarked_cb.setChecked(self.cfg.get("no_global_remark_for_remarked", True))
        gr_layout.addWidget(self.no_global_remark_for_remarked_cb)
        hint_gr = QLabel("程序备注优先高于全局备注")
        hint_gr.setStyleSheet("color:#aaa; font-size:11px;")
        gr_layout.addWidget(hint_gr)
        clayout.addWidget(g_global_remark)
    ...
    def toggle_title_options(self, checked):
        self.title_bg_group.setVisible(checked)

    def toggle_global_remark(self, checked):
        self.global_remark_edit.setEnabled(checked)
        self.global_remark_btn.setEnabled(checked)
    ...
    def choose_font(self):
        font, ok = QFontDialog.getFont(self.font_btn.font(), self, "选择字体")
        if ok:
            self.cfg["font_family"] = font.family()
            self.cfg["font_size"] = font.pointSize()
            self.cfg["font_bold"] = font.bold()
            self.font_label.setText(f"当前：{font.family()} {font.pointSize()}pt {'粗体' if font.bold() else ''}")

    def pick_global_name_bg(self):
        color = QColorDialog.getColor()
        if color.isValid():
            self.cfg["name_bg_color"] = color.name().upper()
            self.global_name_bg_label.setText(f"当前：{self.cfg['name_bg_color']}")

    def open_name_bg_manager(self):
        dlg = NameBgColorManagerDialog(self.custom_name_bg_colors, self)
        if dlg.exec_():
            self.custom_name_bg_colors = dlg.get_colors()

    def set_global_remark(self):
        text = self.global_remark_edit.text().strip()
        if not text:
            QMessageBox.warning(self, "输入错误", "全局备注不能为空")
            return
        self.cfg["global_remark"] = text
    ...
    def save_and_close(self):
        ...
        self.cfg["use_window_title"] = self.cb_use_title.isChecked()
        ...
        self.cfg["show_upload"] = self.cb_upload.isChecked()
        self.cfg["show_download"] = self.cb_download.isChecked()
        ...
        self.cfg["custom_name_bg_colors"] = self.custom_name_bg_colors
        self.cfg["global_remark_enabled"] = self.global_remark_enabled_cb.isChecked()
        self.cfg["global_remark"] = self.global_remark_edit.text().strip()
        self.cfg["no_global_remark_for_remarked"] = self.no_global_remark_for_remarked_cb.isChecked()
...
class NetworkMeter:
    ...
    def get_speed(self):
        try:
            ...
            upload = (current_net.bytes_sent - self.last_net.bytes_sent) / dt
            download = (current_net.bytes_recv - self.last_net.bytes_recv) / dt
...
class Overlay(QWidget):
    ...
    def apply_font(self):
        family = self.cfg.get("font_family", "")
        size = self.cfg.get("font_size", 9)
        bold = self.cfg.get("font_bold", False)
        self.label.apply_config_font(family, size, bold)
    ...
    def update_info(self):
        try:
            ...
            if self.stable_count >= 2:
                ...
                upload, download = self.net_meter.get_speed()

                display_name = title if (self.cfg.get("use_window_title") and title) else name
                name_bg_html = display_name
                custom_colors = self.cfg.get("custom_name_bg_colors", {})
                if name.lower() in {k.lower(): v for k, v in custom_colors.items()}:
                    for k, v in custom_colors.items():
                        if k.lower() == name.lower():
                            bg = v["bgColor"]
                            rad = v["cornerRadius"]
                            name_bg_html = f"<span style='background-color:{bg}; border-radius:{rad}px; padding:2px 6px;'>{display_name}</span>"
                            break
                elif self.cfg.get("name_bg_color") and self.cfg.get("use_window_title"):
                    bg = self.cfg["name_bg_color"]
                    name_bg_html = f"<span style='background-color:{bg}; border-radius:6px; padding:2px 6px;'>{display_name}</span>"
                ...
                global_remark_str = ""
                if self.cfg.get("global_remark_enabled", False):
                    gr = self.cfg.get("global_remark", "").strip()
                    if gr:
                        if not (matched_remark and self.cfg.get("no_global_remark_for_remarked", True)):
                            global_remark_str = f"【{gr}】"

                parts = []
                if global_remark_str:
                    parts.append(global_remark_str)
                if remark_html:
                    parts.append(remark_html)
                if self.cfg.get("show_name") and not (matched_remark and self.cfg.get("remark_hide_name")):
                    parts.append(name_bg_html)
                ...
                if self.cfg.get("show_upload"):
                    parts.append(f"↑:{upload/1024:.1f}KB/s")
                if self.cfg.get("show_download"):
                    parts.append(f"↓:{download/1024:.1f}KB/s")
