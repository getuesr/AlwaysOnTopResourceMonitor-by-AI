...
class SettingsWindow(QWidget):
    ...
    def initUI(self):
        ...
        g_offset_global = QGroupBox("📏 全局悬浮窗偏移")
        g_offset_global.setStyleSheet(g1.styleSheet())
        offset_global_layout = QHBoxLayout(g_offset_global)
        offset_global_layout.addWidget(QLabel("偏移距离:"))
        ...
        offset_global_layout.addWidget(self.global_offset_slider)
        offset_global_layout.addWidget(self.global_offset_label)
...
class Overlay(QWidget):
    ...
    def update_info(self):
        try:
            ...
            # 聚焦警告：如果前台窗口是悬浮窗自身，进入警告模式，但不影响后续监控（直接返回）
